...
export const metadata: Metadata = {
  title: "API Reference — Reverb",
  description:
    "Integrate Reverb into your third-party app. Bearer-token authenticated REST API for generating brand-grounded marketing content.",
...
export default function ApiDocsPage() {
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--reverb-bg-app)" }}>
      <nav className="sticky top-0 z-30 border-b border-[var(--reverb-border-soft)] backdrop-blur-xl bg-white/70">
        <div className="max-w-4xl mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link href="/settings/api-keys" className="h-8 sm:h-9 px-3 sm:px-4 rounded-lg reverb-btn-primary text-[12.5px] sm:text-[13px] font-semibold flex items-center gap-1.5 shrink-0">
      ...
      <article className="relative max-w-3xl mx-auto px-4 sm:px-6 pt-10 sm:pt-16 pb-12 sm:pb-20 w-full">
        ...
        <h1 className="text-[28px] sm:text-[42px] font-semibold tracking-tight text-[var(--reverb-text-primary)] leading-[1.15] mb-4">
          Reverb API Reference
        </h1>
        <p className="text-[15px] sm:text-[17px] text-[var(--reverb-text-secondary)] leading-relaxed mb-8 pb-8 border-b border-[var(--reverb-border-soft)]">
          A REST API for generating brand-grounded marketing content. Authenticate with a
          key from{" "}
          <Link href="/settings/api-keys" className="text-[var(--reverb-accent)] font-medium underline underline-offset-2">
            /settings/api-keys
          </Link>
        ...
        <div className="resource-prose">
          <h2>Base URL</h2>
          <pre><code>https://llm-amits-projects-d3468251.vercel.app/api/v1</code></pre>

          <h2>Authentication</h2>
          <p>
            All requests require an <strong>Authorization</strong> header:
          </p>
          <pre><code>Authorization: Bearer reverb_live_&lt;your-key&gt;</code></pre>
          <p>
            Generate keys at{" "}
            <Link href="/settings/api-keys" className="text-[var(--reverb-accent)] font-medium underline underline-offset-2">
              /settings/api-keys
          ...
          <h2>Example — cURL</h2>
          <pre><code>{`curl https://llm-amits-projects-d3468251.vercel.app/api/v1/chat \\
  -H "Authorization: Bearer reverb_live_YOUR_KEY_HERE" \\
  ...
  -d '{
    ...
    "agent_id": "YOUR_AGENT_UUID"
          ...
          <pre><code>{`const res = await fetch("https://llm-amits-projects-d3468251.vercel.app/api/v1/chat", {
  method: "POST",
  headers: {
  ...
  body: JSON.stringify({
    prompt: "Write 3 LinkedIn ad headlines for our new AI-accelerated data platform.",
    agent_id: process.env.Reverb_AGENT_ID,
  }),
});
...
r = requests.post(
    "https://llm-amits-projects-d3468251.vercel.app/api/v1/chat",
    headers={"Authorization": f"Bearer {os.environ['Reverb_API_KEY']}"},
    json={
        "prompt": "Write 3 LinkedIn ad headlines for our new AI-accelerated data platform.",
        "agent_id": os.environ["Reverb_AGENT_ID"],
    },
    timeout=60,
)
r.raise_for_status()
  ...
  -d '{
    ...
    "messages": [
      ...
      { "role": "user",      "content": "Shorten the sub-headline to under 12 words." }
          ...
          <p>
            When <code>agent_id</code> is provided (and owned by the API key&apos;s user), Reverb injects:
          </p>
          <ul>
          ...
          <p>
            ...
            <Link href="/brand" className="text-[var(--reverb-accent)] font-medium underline underline-offset-2">/brand</Link>.
          </p>

          <h2>Support</h2>
          <p>
            Bug reports and integration questions: <a href="mailto:support@reverb.com" className="text-[var(--reverb-accent)] font-medium underline underline-offset-2">support@reverb.com</a>.
        ...
        <div className="mt-10 sm:mt-14 p-5 sm:p-6 rounded-2xl"
          style={{ background: "var(--reverb-gradient-card)", border: "1px solid var(--reverb-border-soft)" }}>
          <p className="text-[11px] font-bold uppercase tracking-wider text-[var(--reverb-accent)] mb-1.5">
            Ready to build?
          </p>
          <h3 className="text-[18px] sm:text-[20px] font-semibold tracking-tight text-[var(--reverb-text-primary)] mb-2">
            Mint your first key.
          </h3>
          <p className="text-[13px] sm:text-[14px] text-[var(--reverb-text-secondary)] mb-4 leading-relaxed">
            Takes 20 seconds. Copy the plaintext when it&apos;s shown, drop it into your app&apos;s env vars, and you&apos;re integrating.
          </p>
          <Link
            href="/settings/api-keys"
            className="inline-flex items-center gap-2 h-10 px-5 rounded-xl reverb-btn-primary text-[13px] font-semibold"
          >
