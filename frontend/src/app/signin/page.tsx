...
function SignInForm() {
  ...
  return (
    <AuthShell title="Welcome back" subtitle="Sign in to your Reverb account">
      {resetSuccess && (
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex justify-end -mt-1">
          <Link href="/forgot-password" className="text-[12px] text-[var(--reverb-text-secondary)] hover:text-[var(--reverb-accent)] transition-colors">
            Forgot password?
          </Link>
        </div>
        ...
        <button type="submit" disabled={loading || !email || !password}
          className="h-11 rounded-xl reverb-btn-primary text-[14px] font-semibold flex items-center justify-center gap-2 transition-all">
      ...
      <p className="text-center text-[13px] text-[var(--reverb-text-secondary)] mt-6">
        ...
        <Link href="/signup" className="font-semibold text-[var(--reverb-accent)] hover:text-[var(--reverb-accent-rich)]">
          Create one
        </Link>
      </p>
    </AuthShell>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12" style={{ background: "var(--reverb-bg-app)" }}>
      ...
      <div className="relative w-full max-w-md reverb-fade-in">
        ...
        <p className="text-center text-[11px] text-[var(--reverb-text-tertiary)] mt-6 tracking-wide">
  ...
  return (
    <div className="relative">
      ...
      <input type={type} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} autoComplete={autoComplete} required
        className="w-full h-11 pl-10 pr-3.5 rounded-xl bg-white border border-[var(--reverb-border-soft)] text-[14px] text-[var(--reverb-text-primary)] placeholder:text-[var(--reverb-text-tertiary)] focus:outline-none focus:border-[var(--reverb-accent)] focus:ring-4 focus:ring-[var(--reverb-accent)]/10 transition-all" />
    </div>
  );
}
